...
void LCD_Draw_Picture(u16 xstr,u16 ystr,u16 xend,u16 yend,u8 *pic)	    //画图
{
	...
	u32 data;
	LCD_Set_Window(xstr,ystr,(xend-xstr),(yend-ystr));
	LCD_SetCursor(xstr,ystr);
	LCD_WriteRAM_Prepare();
	for(x=0;x<((xend-xstr)*(yend-ystr));x++)
  {	
		data = *(pic+1);
		data = data <<8;
		data = data|*(pic);
...
void LCD_Show_num_xiao(u16 x,u16 y,double num,int leng2,int size)
{
	u32 adcx=0;
	float temp=0;
	float temp2=num;
	...
	if(leng1==0)
		leng1=1;
		adcx=num;                            //赋值整数部分给adcx变量，因为adcx为u16整形
		LCD_ShowxNum(x,y,adcx,leng1,size,0);    //显示电压值的整数部分，3.1111的话，这里就是显示3
		temp=num-adcx;                           //把已经显示的整数部分去掉，留下小数部分，比如3.1111-3=0.1111
		LCD_ShowChar(x+leng1*length,y,'.',size,0);
		{
				int i=0;
				for(i=0;i<leng2;i++)
					temp=temp*10;
			}
		LCD_ShowxNum(x+length+length*leng1,y,temp,leng2,size,0X80); //显示小数部分（前面转换为了整形显示），这里显示的就是111.
}
